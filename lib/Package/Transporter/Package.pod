...
=item $pkg->B<enumerated_values>('ATB_', [], $iterator, qw(STOCK  PRICE));

Enumerate values for the symbols in ascending order. Calling parameters are a name prefix, the symbol properties, an iterator for automatic counting, and the names.
...
A symbol has the following properties. Values marked with an asterisk
are the default. The values are only available after Transporter has
been use'd, which means you need a separate BEGIN block (see
examples/CF4-long_definition.pl).
...
Create managed attributes (get_/set_-style) from the symbols, suitable for array based object (abo). Calling parameter are the method name, the application properties, and the symbol names or prefixes (indicated by trailing _). The ugly name indicates that it is experimental.
...
An application has the following properties. Values marked with an asterisk are the default. The values are only available after Transporter has
been use'd, which means you need a separate BEGIN block (see
examples/CF4-long_definition.pl).
...
=item $pkg->B<array_indices>('ATB_', [], qw(NAME  PRICE));
...
=item $pkg->B<single_bit_masks>('FLAG_', [], qw(MOVING  TURNING));

Like array indices, but with an enumerator to multiply by two, not to add one. 

=item $pkg->B<package_constants>('IS_', qw(TRUE  FALSE));

A combination of named_values and constant_function with SCP_PUBLIC and MIX_IMPLICIT set. Calling parameter are a name prefix, and the names. 
...
=item $pkg->B<mix_along_hierarchy>();

Search through the own package name for other packages using Transporter; package A:B:C will attempt to mix in from A:B and A. No calling parameter; no return value.

=item $pkg->B<mix_along_isa>();
...
=item $pkg->B<mix_explicit>('CF5', qw(ATB_PRICE));
