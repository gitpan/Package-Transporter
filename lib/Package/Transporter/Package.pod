...
Whether a merge will have to specify the application explicitly or not.
...
=item B<Propagation> - PRP_LOCAL(*) / PRP_UNIVERSAL

Whether the application should be propagated to the local package or
to the special package '*', which holds universal applications.
