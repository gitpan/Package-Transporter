...
completing the OO system of Perl.
...
OO support in Perl, meaning identifier management for methods plus
attributes. With Perl6 or any modern language, Package::Transporter has no
point. If you plan to switch to Perl6 soon, then Package::Transporter is
definitely not for you.


=head1 DESCRIPTION

The objective of Package::Transporter is to encourage a greater
...
If this package is convincing, it will result in many symbols being
used. That introduces the chance of name clashes. Meaning that the
name is allocated for conflicting purposes. You can protect from such
...
recommendation and independant of the scope or propagation of the
name.
...
that is unpredictable - maybe it never happens - and too late compared to the
trivial nature of the error.
...
Arrays are faster and require less memory than hashes in
perl5. Package::Transporter makes it attractive to implement objects as arrays.
...
amount of code and functions provided; although you might find them
less complex and easier to understand. With Package::Transporter, the symbol
...
constant_function applied. One example in the man page really calls
for more compactness, which can be provided with Package::Transporter
(see example RELCON1 below).
...
When you activate the Package::Transporter module through the statement 'use', pass
a package visiting point as a parameter. Meaning the somewhat funny
anonymous subroutine sub{eval shift}. All actions of Package::Transporter refer
to the visiting point. You can re-define the visiting point of a package
by passing such a subroutine to the constructor again, like in

	my $pkg = Package::Transporter->new(sub{eval shift});

Remember that the new method returns an object of class
Package::Transporter::Package, therefore the variable name $pkg. Upon
activation of Package::Transporter for your package, it will automatically
implement symbols, which have previously been declared with a
propagation of 'universal'. Example of defining the universal symbols
YES and NO:
...
If you don't want to receive universal Package::Transporter symbols, then don't
pass the visiting point upon activation.

Further details on the usage can be found in the documentation for the package Package::Transporter::Package.
...
Winfried Trumper <pub+perl@wt.tuxomania.net>
