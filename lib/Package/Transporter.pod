...
is about compile-time.
	...
	use Package::Transporter sub{eval shift}, 'mix_in:*', sub {
		$_[0]->array_indices('ATB_', [], qw(NAME  SALE  STOCK  PRICE));
	};
...
older than Perl, but not commonly found in Perl.
	...
	use Package::Transporter sub{eval shift}, sub {
		$_[0]->class_constants('IS_', 'ON_SALE' => 1);
	};
...
Optional arguments in the use statement can specify the mix
...
The default visit point should be the loading of the
Package::Transporter module via use, but that isn't enforced. Upon
loading, Package::Transporter will create an object for the calling
package to store package details like the visit point. The object is
monadic within a package and of type
Package::Transporter::Package. Meaning that
'Package::Transporter->new' won't give you a fresh object, but rather
the one belonging to the current package.
...
This is the first public release. Also see the Todo file.

Subroutine attributes are not recognized. While setting such attributes is easy, getting hold of them is difficult to get right. Which might be the reason why they're not widely used.

Transporter stores data in package attributes, which aren't covered when you serialize an object with Data::Dumper or similar tools. With regards to its own data, persistence isn't a concept built into Transporter. The data of Transporter is considered a temporary affair of the compilation phase, although you are free to use it afterwards.

Perl objects implemented via array references can be serialized and loaded later as long as the structure of the attributes doesn't change. Note that the enumeration of attributes is done in the order their names are given. If you change the order, you change the structure. Even appending a new name at the end of the existing attributes might change the structure, because it changes enumeration in derived classes. This limitation is highlighted, because Perl objects are traditionally implemented as hash references.
